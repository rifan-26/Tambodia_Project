...
class ScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Add pagination to prevent timeout - limit to 50 records per page
        $media = $query->with(['schedules' => function($q) {
                $q->orderBy('start_date', 'desc')->limit(10); // Limit schedules per media
            }])
            ->orderBy('created_at', 'desc')
            ->limit(100) // Limit total media items
            ->get();
        ...
        // Get all media that can be used as background with limit
        $backgroundMedia = Media::whereIn('type', ['Gambar', 'Video'])
            ->orderBy('created_at', 'desc')
            ->limit(50)
            ->get();
        
        // Get scheduled backgrounds and descriptions with limit
